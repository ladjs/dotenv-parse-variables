...
# note a string between quotes won't be parsed
...
```js
{
  ...
  PING: [ 'ping', 'true', 2, 100 ],
  // NOTE: this was not parsed because the string was between quotes
  PONG: 'some,thing,that,goes,wow'
}
```

If your configuration line ends in `*` it will not be parsed by this package, which allows you to keep values as the `String` variable type if needed. Also when you encapsulate a value between quotes e.g. "value", the value won't be parsed and it will return as a `String` variable. This can be used in situations where you for example have a ',' inside your string and it should not be parsed as an array.
